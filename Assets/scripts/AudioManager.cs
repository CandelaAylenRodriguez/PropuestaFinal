...
public class AudioManager : MonoBehaviour
{
    ...
    void Awake()
    {
        Debug.Log($"[AudioManager] Awake ejecutado - nombre del objeto: {gameObject.name}");

        if (Instance == null)
        {
            Instance = this;
            DontDestroyOnLoad(gameObject);
            Debug.Log("[AudioManager] Instancia creada y marcada como persistente");
        }
        else
        {
        ...
        float savedMusica = PlayerPrefs.GetFloat("VolumenMusica", 0.7f);
        float savedSFX = PlayerPrefs.GetFloat("VolumenSFX", 1f);

        // IMPORTANTE: Usar setters para asegurar que volumen + PlayerPrefs estén en sincronía
        AjustarVolumenMusica(savedMusica);
        AjustarVolumenEfectos(savedSFX);
    }
